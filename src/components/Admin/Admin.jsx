...
const Admin = () => {
  ...
  const fetchLevels = async () => {
    try {
      ...
      const response = await axios.get(
        `https://fitsafar.onrender.com/api/levels?page=${currentPage}&limit=10${
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingLevel) {
        await axios.post(`https://fitsafar.onrender.com/api/levels/${editingLevel}`, formData);
      } else {
        await axios.post(`https://fitsafar.onrender.com/api/levels`, formData);
      }
  ...
  const handleDelete = async (levelNumber) => {
    if (window.confirm('Are you sure you want to delete this level?')) {
      try {
        await axios.delete(`https://fitsafar.onrender.com/api/levels/${levelNumber}`);
        fetchLevels();
      } catch (error) {
