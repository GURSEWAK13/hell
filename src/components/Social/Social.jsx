  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        ...
        {post.imageUrl && (
          <img
            src={`https://fitsafar.onrender.com${post.imageUrl}`}
            alt="Post content"
...
const fetchPosts = async () => {
    try {
      ...
      const response = await axios.get(
        `https://fitsafar.onrender.com/api/posts?${params.toString()}`,
        {
...
useEffect(() => {
  ...
  const handleSubmitPost = async (e) => {
    ...
    try {
      ...
      if (editingPost) {
        await axios.put(`https://fitsafar.onrender.com/api/posts/${editingPost._id}`, formData, {
          headers: { 
        ...
        await axios.post(`https://fitsafar.onrender.com/api/posts`, formData, {
          headers: { 
            Authorization: `Bearer ${token}`,
            'Content-Type': 'multipart/form-data'
          }
  ...
  const handleLike = async (postId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://fitsafar.onrender.com/api/posts/${postId}/like`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchPosts();
    } catch (error) {
  ...
  const handleComment = async (postId, commentText) => {
    try {
      ...
      const response = await axios.post(
        `https://fitsafar.onrender.com/api/posts/${postId}/comments`,
        { text: commentText },
        {
          headers: { 
            'Authorization': `Bearer ${token}`,
  ...
  const handleDeletePost = async (postId) => {
    if (window.confirm('Are you sure you want to delete this post?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://fitsafar.onrender.com/api/posts/${postId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
